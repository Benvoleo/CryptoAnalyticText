...
namespace LabFomrs1_Version2
{
    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            try
            {
                using (WordprocessingDocument document = WordprocessingDocument.Open("pro-avtorske-pravo-i-sumizhni-prava.docx", true))
                {
                    inputedStr = document.MainDocumentPart.Document.Body.InnerText;
                    textBox1.Text += inputedStr;
            ...
            label1.Text = "Кол-во символов: "+ inputedStr.Length;
            ...
            double sum = 0;
            countOfSingle = countOfSingle.OrderBy(x => x.Value).ToDictionary(x => x.Key, x => x.Value);
            ...
            {
                if (Char.IsLetter(charStr[i]) && Char.IsLetter(charStr[i+1]))
                {
            ...
            countOfCoupe = countOfCoupe.OrderBy(x => x.Value).ToDictionary(x => x.Key, x => x.Value);
         ...
         void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                // Create a file to write to.
                string createText = textBox1 + Environment.NewLine  + textBox3 + Environment.NewLine + textBox4 + Environment.NewLine;
                File.WriteAllText(path, createText);
            }
            else
            {
